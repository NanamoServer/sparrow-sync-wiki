...
function ensureMobileSocialLinks() {
  ...
  // Mirror the social links actually present in the navbar (Discord, QQ, ...).
  // No hardcoded fallbacks: if the navbar has none of these links configured,
  // no mobile social icons are injected at all.
  const specs = [
    {className: 'navbar-discord-link', label: 'Discord'},
    {className: 'navbar-qq-link', label: 'QQ'},
  ]
    .map((spec) => {
      const orig = document.querySelector(`.navbar__items a.${spec.className}`);
      const url = orig?.getAttribute('href');
  ...
  for (const {className, label, url} of specs) {
